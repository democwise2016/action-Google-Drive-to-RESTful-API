...
RUN npm link csv-parser@3.0.0
RUN npm link axios@1.7.2

# RUN apt-get update
# RUN apt-get install -y wget default-jdk
# RUN mkdir -p /java/
# WORKDIR /java
# RUN wget https://downloads.apache.org/tika/2.9.2/tika-app-2.9.2.jar
# WORKDIR /
...
# RUN mv pdf2htmlEX-0.18.8.rc1-master-20200630-Ubuntu-bionic-x86_64.AppImage pdf2htmlEX.AppImage
...
RUN apt update
RUN apt install -y libfontconfig1 libcairo2 libjpeg62-turbo-dev
...
RUN apt-get install -y libjpeg-turbo8-dev libjpeg-turbo8=1.4.2-0ubuntu3
RUN apt install -y ./pdf2htmlEX.deb